...
### 🔮 주가 예측
- FinanceDataReader를 사용해 최근 6개월간 데이터를 가져와 LSTM 모델로 학습
- 향후 5일간 종가 예측 (B=영업일 기준)
- 예측 결과를 텍스트 및 차트로 시각화

### 📊 예측 정확도 평가
- 최근 5일간의 실제 종가와 예측값 비교
- MAE (Mean Absolute Error), RMSE (Root Mean Squared Error) 출력
- 비교 결과를 시각화 (예측 vs 실제 그래프)

### 💰 모의 투자 시뮬레이션
- 예측된 주가를 바탕으로 매수/매도 실행
- 보유 종목 및 손익 실시간 표시
- 다음날 버튼으로 예측일 순차 진행

### 💾 상태 저장
- 현재 자산, 보유 주식, 예측 결과를 `portfolio.json` 파일에 저장
- GUI 재시작 시 상태 복원
